...
def spreadsheetToList(ss: str) -> List[List[int]]:
    rows = ss.split("\n")
    rows = [[int(cell) for cell in row.split()] for row in rows]
    return rows

def calcChecksum(s: int):
    rows = spreadsheetToList(s)
    total = sum(max(row)-min(row) for row in rows)
...
7 5 3
2 4 6 8"""
assert calcChecksum(test_ss) == 18
